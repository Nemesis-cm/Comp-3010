...
| If (e,e1,e2) ->
    ...
    | Apply(Lambda(var1, typ1, body1), Lambda(var2, typ2, body2)) ->
        ...
        | Apply(e1, Lambda(var,typ,body)) -> raise Type_error
        | Apply (e1,e2) -> 
    ...
    let rec string_of_fun (e : exp) = 
        ...
        | Plus(left, right) ->
        "(" ^ string_of_fun left ^ " , " ^ string_of_fun right ^ ")"
    ...
    let rec print_list_string myList = match myList with
        ...
        let print_substitution e1 x e2 = print_endline(string_of_fun(substitution e1 x e2));;
